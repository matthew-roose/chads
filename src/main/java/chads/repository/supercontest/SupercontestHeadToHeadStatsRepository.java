...
public interface SupercontestHeadToHeadStatsRepository
    ...
    @Query(value = "SELECT * FROM sc_head_to_head_stats WHERE (username = :user1 OR username = :user2) AND " +
            "game_id IN (SELECT game_id FROM supercontest_entry_week sew JOIN supercontest_pick sp " +
            "ON sp.entry_week_id = sew.id " +
